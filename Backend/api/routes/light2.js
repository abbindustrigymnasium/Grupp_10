...
router.post('/', (req, res) => { 
    //  '/'= indexfilen i localhost/products/   req = request , res= resultat , next = det som ska hända senare 
...
//Tog bort res.status(201) då dubbla status skickningar skapar error,
 //vill man ha möjlighet till bägge så gör if else satser och lägg dem i.
    }).catch(err => {
        console.log(err);
        res.status(500).json({
            error: err
...
router.patch('/', (req, res) => { 
    ...
    var createProduct = function () { //Skapar funktionen
        return new Promise(function (resolve, reject) { //Skapar löftet
            con.query('UPDATE light SET `hard` = ? , `strength` = ? Where `name` = ?',
                      [Lights[0], Lights[1], Lights[2]], function (err, result) {
                if (err) {
                    return reject(err);
                } else {
                    return resolve(Lights);
